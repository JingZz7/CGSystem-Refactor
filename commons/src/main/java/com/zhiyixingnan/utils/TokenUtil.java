...
public class TokenUtil {
  ...
  public static String sign(String id) {
    ...
    String token = null;
    try {
      Date expiresAt = new Date(System.currentTimeMillis() + TokenUtil.EXPIRE_TIME);
      token =
          JWT.create()
              .withIssuer("auth0")
              .withClaim("id", id)
              ...
              // 使用了HMAC256加密算法。
              .sign(Algorithm.HMAC256(TokenUtil.TOKEN_SECRET));
    } catch (Exception e) {
      e.printStackTrace();
    }
    return token;
  ...
  public static List<String> verify(String token) {
    ...
    try {
      JWTVerifier verifier =
          JWT.require(Algorithm.HMAC256(TokenUtil.TOKEN_SECRET)).withIssuer("auth0").build();
